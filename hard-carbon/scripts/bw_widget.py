...
sublatt1, = ax4.plot(df_bw['x'], df_bw['n1'], lw=2,label ='Pores',color='orange') # Initial plt
sublatt2, = ax4.plot(df_bw['x'], df_bw['n2'], lw=2,label ='Layers',color='black') # Initial plt
...
seta = Slider(axeta, 'Cap (mAh/g)', 180.0, 500.0, valinit = initial_cap, valstep = 10.0)
setE01 = Slider(axE01, 'E0,1 (eV)', -1.0, 0.2, valinit = -0.25, valstep = delta_eta)
setbeta = Slider(axbeta, 'beta', 0.25, 3.0, valinit = 2.0, valstep = delta_eta)
setalpha = Slider(axalpha, 'alpha (eV)', -2.5, -0.5, valinit = -1.0, valstep = delta_eta) 
setdelE =  Slider(axdelE, 'delE (eV)', -1.0, 0.0, valinit = -0.25, valstep = delta_eta)
setL =  Slider(axL, 'L', 0.10, 0.90, valinit = 0.25, valstep = delta_eta)
setg =  Slider(axg, 'g (eV)', -0.07, 0.02, valinit = -0.047, valstep = 0.002) 
...
ax7.plot(df_dqdv['OCV'],-df_dqdv['dQdV'],label='Experiment',color='tomato')
