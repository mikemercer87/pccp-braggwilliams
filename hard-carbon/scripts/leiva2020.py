 ...
 #   print Omega(j,N)
    ...
    def S(self,N):
        if N == 0 or N == self.M1 + self.M2:
            ...
            if N < self.M:
                beta_array = np.array([self.beta*self.E(j, N) for j in range(0 , N + 1)])
                boltzmanns = np.sum([beta_array[j] * np.exp(self.logOmega(j, N) - beta_array[j] - self.logQ_array[N]) for j in range(N + 1)])
            else:
                beta_array = np.array([self.beta*self.E(j, N) for j in range(0 , self.M1 + self.M2 -N + 1)])                
                boltzmanns = np.sum([beta_array[j] * np.exp(self.logOmega(j, N) - beta_array[j] - self.logQ_array[N]) for j in range(self.M1 + self.M2 - N + 1)]) 
            result= (boltzmanns + self.logQ_array[N]) # Shifted Q_array index.
