...
#            smooth_dqdv = interp1d(x = soc_array.tolist(), y = dqdv_array.tolist(), kind='linear')            
    ...
    def read_files(self, key='Galvan_C50', cells = 'all'):
        ...
        self.df = pd.read_csv(self.galvan_path+'hard_carbon_3cycles_03.txt',skiprows=12,sep=',')
#        self.df = pd.read_csv('galvanostatic/HC_formation_00.txt',skiprows=12,sep=',')        
    ...
    def plot(self,kind='sodiation'):
        ...
        for window in [1250]:
            ...
            for index, count in enumerate(range(3,self.df['Count'].max()+1)):
...
#                   self.df_cyc = self.df_cyc[self.df_cyc['I[A]'] > -1e-9] # Charge cycles only.                                        
...
#                    plt.plot(self.dqdv_red['U[V]'].iloc[1:-1], -self.dqdv_red['dqdv'].iloc[1:-1], label='cycle='+str(count)+', ' + kind,color=colors[index],linestyle=linestyles[index])
                    plt.plot(self.dqdv_red['U[V]'].iloc[1:-1], -self.dqdv_red['dqdv'].iloc[1:-1], label='cycle='+str(count)+', ' + kind,color='b',linestyle=linestyles[index])
                else:
                    plt.plot(self.dqdv_red['U[V]'].iloc[1:-1], -self.dqdv_red['dqdv'].iloc[1:-1], label='cycle='+str(count)+', ' + kind,color='orange',linestyle=linestyles[index])
                             
                plt.ylabel('dQ/dV (arb units)')
                plt.xlabel('Voltage (V) vs. Na')
...
if __name__ == '__main__':
    dqdv = DQDV()
    dqdv.plot(kind='sodiation')
    dqdv.plot(kind='desodiation')
 #   dqdv.OCV_to_dQdV()
    plt.legend()
    plt.savefig('plot_formation.png',dpi=500)
    plt.show()    
